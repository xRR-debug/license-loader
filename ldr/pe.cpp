...
)
{
	*FileHandle = NULL;
	...
	InitializeObjectAttributes(&ObjectAttributes,
		...
		PathIsRelative ? RelativeName.ContainingDirectory : NULL,
		NULL);
...
)
{
	...
	*ViewSize = 0;
	...
	if (!NT_SUCCESS(Status))
		printf("NtMapViewOfSection: 0x%08X\n", Status);
...
)
{
	...
	if (FunctionRva >= ExportDirRva && FunctionRva < ExportDirRva + ExportDirSize)
		return NULL; // Ignore forwarded exports
