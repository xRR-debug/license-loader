...
string CLicense::PostUrlData(string url)
{
	string request_data = "";

	HINTERNET hIntSession = InternetOpenW(_xor_(L"").c_str(), INTERNET_OPEN_TYPE_DIRECT, NULL, NULL, 0);
	...
	HINTERNET hHttpSession = InternetConnectA(hIntSession, HOST, 80, NULL, NULL, INTERNET_SERVICE_HTTP, 0, NULL);
	...
	HINTERNET hHttpRequest = HttpOpenRequest(hHttpSession, _xor_("POST").c_str(), url.c_str()
	...
	DeleteUrlCacheEntry(HOST);
	DeleteUrlCacheEntry(PATH);
...
string CLicense::GetUrlData(string url)
{
	...
	HINTERNET hHttpSession = InternetConnect(hIntSession, HOST, 80, NULL, NULL, INTERNET_SERVICE_HTTP, 0, NULL);
	...
	HINTERNET hHttpRequest = HttpOpenRequestA(hHttpSession, _xor_("GET").c_str(), url.c_str()
		, 0, 0, 0, INTERNET_FLAG_RELOAD, 0);
...
string CLicense::GetMacID()
{
	...
	do
	{
		sprintf_s(pszMacAddy, 64, xorstr_("%02X-%02X-%02X-%02X-%02X-%02X"),
...
{
	string SerialKey = xorstr_("61A345B5496B2");
...
string CLicense::GetSerial()
{
	...
	string Serial1 = HashSerialKey.substr(0, 5);
	string Serial2 = HashSerialKey.substr(5, 5);
	string Serial3 = HashSerialKey.substr(10, 5);
	string Serial4 = HashSerialKey.substr(15, 5);
...
string CLicense::GetUserDayCount()
{
	...
	UrlRequest.append(_xor_("gate.php?day=").c_str() + Serial);
...
bool CLicense::CheckLicenseURL(string URL, string GATE, string KEY)
{
	...
	if (ReciveHash.size())
	{
		...
		for (int RandomSha256 = 4; RandomSha256 <= 40; RandomSha256++)
		{
			string LicenseCheck = LicenseOK + to_string(RandomSha256);
...
{
	string UrlRequest = PATH;
	UrlRequest.append(_xor_("gate.php?version=ok").c_str());

	string GetCheatVersion = GetUrlData(UrlRequest);

	if (GetCheatVersion == _xor_("error: 1").c_str())
	{
		return false;
	}
...
std::string CLicense::GetVersion()
{
	...
	UrlRequest.append(_xor_("gate.php?version=ok").c_str());
	return GetUrlData(UrlRequest);
} 

std::string CLicense::GetActivatedKey(char inuserkey[16])
{
	std::string Serial = GetSerial64();

	std::string userkey = inuserkey;

	std::string UrlRequest = PATH;

	std::string vk = _xor_("loader_call").c_str();

	UrlRequest.append(_xor_("regkey.php?userkey=").c_str() + userkey + _xor_("&hwid=").c_str() + Serial + _xor_("&vk=").c_str() + vk);

	static std::string ReciveIF = GetUrlData(UrlRequest);
	return ReciveIF;
}

void CLicense::ShowUpdateUrl()
{
	LI_FN(MessageBoxA).get()(0, _xor_("New version available! :>").c_str(), _xor_("Update").c_str(), MB_OK | MB_ICONINFORMATION);

	string UpdateUrl;

	UpdateUrl.append(_xor_("https://").c_str());
	UpdateUrl.append(HOST);
	UpdateUrl.append(PATH);
	UpdateUrl.append(_xor_("update.php?update=").c_str());
	UpdateUrl.append(GetSerial64());

	string url = _xor_("url.dll,FileProtocolHandler ").c_str();
	url.append(UpdateUrl);

	ShellExecute(NULL, _xor_("open").c_str(), _xor_("rundll32.exe").c_str(), url.c_str(), NULL, SW_SHOWNORMAL);
